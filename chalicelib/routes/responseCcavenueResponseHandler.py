...
def response_ccavenue_response_handler(responseId):
  ...
  if not config:
    mark_error_payment(response, f"CCAvenue config not found for merchant id: {merchant_id}.", "ccavenue", paramDict)
  ...
  if paramDict["merchant_param1"] != formId or paramDict["merchant_param2"] != responseId:
    mark_error_payment(response, f"Form id / response id do not match. \nExpected: {formId}/{responseId}. \nReceived: {paramDict['merchant_param1']}/{paramDict['merchant_param1']}", "ccavenue", paramDict)
  if paramDict["order_status"] != "Success":
    mark_error_payment(response, f"Order status does not mark success. Expected: Success. Received: {paramDict['order_status']}", "ccavenue", paramDict)
    ...
    response.save()
    redirect_url = get(form.formOptions.paymentMethods, "ccavenue.redirectUrl", "http://www.chinmayamission.com")
