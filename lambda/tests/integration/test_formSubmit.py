...
ONE_SUBMITRES = {'paid': False, 'success': True, 'action': 'insert', 'email_sent': False, 'paymentInfo': {'currency': 'USD', 'items': [{'amount': 0.5, 'description': 'Base Registration', 'name': 'Base Registration', 'quantity': 1.0, 'total': 0.5}], 'total': 0.5}, 'paymentMethods': {'paypal_classic': {
    'address1': '123', 'address2': 'asdad', 'business': 'aramaswamis-facilitator@gmail.com', 'city': 'Atlanta', 'cmd': '_cart', 'email': 'success@simulator.amazonses.com', 'first_name': 'Ashwin', 'image_url': 'http://www.chinmayanewyork.org/wp-content/uploads/2014/08/banner17_ca1.png', 'last_name': 'Ash', 'payButtonText': 'Pay Now', 'sandbox': False, 'state': 'GA', 'zip': '30022'}}}
...
class FormSubmit(BaseTestCase):
    ...
    def setUp(self):
        ...
        self.edit_form(self.formId, {
                       "schema": ONE_SCHEMA, "uiSchema": ONE_UISCHEMA, "formOptions": ONE_FORMOPTIONS})
    ...
    def test_submit_form_proper_recurrence_paymentInfo(self):
        ...
        self.edit_form(self.formId, {"schema": {"a": "B"}, "uiSchema": {
    ...
    def test_submit_form_coupon_codes_not_successful(self):
        ...
        formOptions = {"paymentInfo": {"items": [
            {"amount": "1", "quantity": "1", "name": "Name",
                "description": "Description"},
            {"amount": "-1 * $total", "quantity": "couponCode:CODE", "couponCode": "CODE",
                "name": "Coupon Code Name", "description": "Coupon Code Description"}
        ]}}
        self.edit_form(
    ...
    def test_submit_form_coupon_codes_partial_off_successful(self):
        ...
        formOptions = {"paymentInfo": {"items": [
            {"amount": "1", "quantity": "1", "name": "Name",
                "description": "Description"},
            {"amount": "-0.5 * $total", "quantity": "$couponCode:CODE", "couponCode": "CODE",
                "name": "Coupon Code Name", "description": "Coupon Code Description"}
        ]}}
        self.edit_form(
            formId, {"schema": schema, "uiSchema": uiSchema, "formOptions": formOptions})
    ...
    def test_submit_form_coupon_codes_constant_off_successful(self):
        ...
        formOptions = {"paymentInfo": {"items": [
            {"amount": "1", "quantity": "1", "name": "Name",
                "description": "Description"},
            {"amount": "-0.2", "quantity": "$couponCode:CODE", "couponCode": "CODE",
                "name": "Coupon Code Name", "description": "Coupon Code Description"}
        ]}}
        self.edit_form(
            formId, {"schema": schema, "uiSchema": uiSchema, "formOptions": formOptions})
    ...
    def test_submit_form_coupon_codes_limit_failure(self):
        ...
        formOptions = {"paymentInfo": {"items": [
            {"amount": "1", "quantity": "1", "name": "Name",
                "description": "Description"},
            {"amount": "-0.2", "quantity": "$couponCode:CODE", "couponCodeMaximum": "0",
                "couponCode": "CODE", "name": "Coupon Code Name", "description": "Coupon Code Description"}
        ]}}
        self.edit_form(
            formId, {"schema": schema, "uiSchema": uiSchema, "formOptions": formOptions})
    ...
    def test_submit_form_coupon_codes_limit_failure_with_amount_total(self):
        ...
        formOptions = {"paymentInfo": {"items": [
            {"amount": "1", "quantity": "1", "name": "Name",
                "description": "Description"},
            {"amount": "-1 * $total", "quantity": "$couponCode:CODE", "couponCodeMaximum": "0",
                "couponCode": "CODE", "name": "Coupon Code Name", "description": "Coupon Code Description"}
        ]}}
        self.edit_form(
            formId, {"schema": schema, "uiSchema": uiSchema, "formOptions": formOptions})
    ...
    def test_submit_form_coupon_codes_limit_success(self):
        ...
        formOptions = {"paymentInfo": {"items": [
            {"amount": "1", "quantity": "1", "name": "Name",
                "description": "Description"},
            {"amount": "-0.2", "quantity": "$couponCode:CODE", "couponCodeMaximum": "5",
                "couponCode": "CODE", "name": "Coupon Code Name", "description": "Coupon Code Description"}
        ]}}
        self.edit_form(
            formId, {"schema": schema, "uiSchema": uiSchema, "formOptions": formOptions})
    ...
    def test_submit_form_coupon_codes_limit_with_count_fail(self):
        ...
        formOptions = {"paymentInfo": {"items": [
            {"amount": "1", "quantity": "1", "name": "Name",
                "description": "Description"},
            {"amount": "-0.2", "quantity": "$couponCode:CODE", "couponCodeMaximum": "2", "couponCodeCount": "$participants",
                "couponCode": "CODE", "name": "Coupon Code Name", "description": "Coupon Code Description"}
        ]}}
        self.edit_form(
            formId, {"schema": schema, "uiSchema": uiSchema, "formOptions": formOptions})
    ...
    def test_submit_form_coupon_codes_limit_with_count_success(self):
        ...
        formOptions = {"paymentInfo": {"items": [
            {"amount": "1", "quantity": "1", "name": "Name",
                "description": "Description"},
            {"amount": "-0.2", "quantity": "$couponCode:CODE", "couponCodeMaximum": "10", "couponCodeCount": "$participants",
                "couponCode": "CODE", "name": "Coupon Code Name", "description": "Coupon Code Description"}
        ]}}
        self.edit_form(
            formId, {"schema": schema, "uiSchema": uiSchema, "formOptions": formOptions})
    ...
    def test_submit_form_postprocess(self):
        formId = self.create_form()
        schema = {"properties": {"custom": {"type": "string"}, "participants": {
        ...
        self.assertEqual(submit_res['success'], True)
        self.assertEqual(submit_res['value'], {"participants": [
                         "a", "b", "c"], "custom": 3, "custom2": 3, "date1": "2000-10-20", "date2": "2000-11-20"})
        self.delete_form(formId)
    ...
    def test_edit_response(self):
        """Create form."""
        self.formId = self.create_form()
        self.edit_form(self.formId, {"schema": ONE_SCHEMA, "uiSchema": ONE_UISCHEMA, "formOptions": dict(
            ONE_FORMOPTIONS, loginRequired=True)})
        ...
        self.assertEqual(submit_res.pop("value"), ONE_FORMDATA)
        self.assertEqual(submit_res, {'paid': False, 'amt_received': {'currency': 'USD', 'total': 0.0}, 'success': True, 'action': 'update', 'email_sent': False, 'paymentInfo': {'currency': 'USD', 'items': [{'amount': 0.5, 'description': 'Base Registration', 'name': 'Base Registration', 'quantity': 1.0, 'total': 0.5}], 'total': 0.5}, 'paymentMethods': {'paypal_classic': {
                         'address1': '123', 'address2': 'asdad', 'business': 'aramaswamis-facilitator@gmail.com', 'city': 'Atlanta', 'cmd': '_cart', 'email': 'success@simulator.amazonses.com', 'first_name': 'Ashwin', 'image_url': 'http://www.chinmayanewyork.org/wp-content/uploads/2014/08/banner17_ca1.png', 'last_name': 'Ash', 'payButtonText': 'Pay Now', 'sandbox': False, 'state': 'GA', 'zip': '30022'}}})
        ...
        self.assertEqual([remove_date(i) for i in body['res']['response']['update_trail']], [{'old': {'contact_name': {'first': 'Ashwin', 'last': 'Ash'}, 'address': {'line1': '123', 'line2': 'asdad', 'city': 'Atlanta', 'state': 'GA', 'zipcode': '30022'}, 'email': 'success@simulator.amazonses.com', 'phone': '1231231233', 'amount': 0.5, 'subscribe': True}, 'new': {'contact_name': {'first': 'Ashwin', 'last': 'Ash'}, 'address': {
                         'line1': '123', 'line2': 'asdad', 'city': 'Atlanta', 'state': 'GA', 'zipcode': '30022'}, 'email': 'success@simulator.amazonses.com', 'phone': '1231231233', 'amount': 0.5, 'subscribe': True}, 'update_type': 'update', '_cls': 'chalicelib.models.UpdateTrailItem'}, {'path': 'contact_name.last', 'user': 'cm:cognitoUserPool:1e3aa7b7-b042-4834-98f1-7915985c39a5', 'old_value': 'Ash', 'new_value': 'NEW_LAST!', 'response_base_path': 'value', '_cls': 'chalicelib.models.UpdateTrailItem'}])
    ...
    def test_edit_response_admin_info(self):
        ...
        self.assertEqual([remove_date(i) for i in body['res']['response']['update_trail']], [{'old': {'contact_name': {'first': 'Ashwin', 'last': 'Ash'}, 'address': {'line1': '123', 'line2': 'asdad', 'city': 'Atlanta', 'state': 'GA', 'zipcode': '30022'}, 'email': 'success@simulator.amazonses.com', 'phone': '1231231233', 'amount': 0.5, 'subscribe': True}, 'new': {'contact_name': {'first': 'Ashwin', 'last': 'Ash'}, 'address': {
                         'line1': '123', 'line2': 'asdad', 'city': 'Atlanta', 'state': 'GA', 'zipcode': '30022'}, 'email': 'success@simulator.amazonses.com', 'phone': '1231231233', 'amount': 0.5, 'subscribe': True}, 'update_type': 'update', '_cls': 'chalicelib.models.UpdateTrailItem'}, {'path': 'orderId', 'user': 'cm:cognitoUserPool:1e3aa7b7-b042-4834-98f1-7915985c39a5', 'old_value': '123', 'new_value': '123', 'response_base_path': 'admin_info', '_cls': 'chalicelib.models.UpdateTrailItem'}])
    ...
    def test_mark_successful_payment(self, mock_boto_client):
        ...
        self.assertEqual(response['payment_trail'], [{'value': {'a': 'b', 'c': 'd'}, 'method': 'unittest_ipn',
                                                      'status': 'SUCCESS', 'id': 'payment1', '_cls': 'chalicelib.models.PaymentTrailItem'}])
        self.assertEqual(response['paid'], True)
        self.assertEqual(response['amount_paid'], "0.5")
    ...
    def test_mark_successful_payment_2(self, mock_boto_client):
        ...
        response['payment_trail'][0].pop("date_created")
        response['payment_trail'][0].pop("date_modified")
        self.assertEqual(response['payment_trail'], [{'value': {'a2': 'b2', 'c2': 'd2'}, 'method': 'unittest_ipn2',
                                                      'status': 'SUCCESS', 'id': 'payment2', '_cls': 'chalicelib.models.PaymentTrailItem'}])
        self.assertEqual(response['paid'], True)
