...
class TestCreateEmail(unittest.TestCase):
    ...
    def test_create_email_html_template_with_hardcoded_to(self):
        res = create_confirmation_email_dict(
            ...
            dict(
                CONFIRMATION_EMAIL_INFO_TEMPLATE,
                to=[
                    "email1@chinmayamission.com",
                    "email2@chinmayamission.com",
                    "email3@chinmayamission.com",
                ],
            ),
        )
        self.assertEqual(
    ...
    def test_create_email_html_template_with_invalid_toField(self):
        ...
        self.assertEqual(res["toEmail"], ["success@simulator.amazonses.com"])
...
{% endfor %}""".replace(
        ...
        self.assertEqual(
            fill_string_from_template(
                Response(amount_paid="0.0", paymentInfo={"items": [{"total": -100}]}),
                template,
            ),
            "True",
        )
        self.assertEqual(
            fill_string_from_template(
                Response(amount_paid="0.1", paymentInfo={"items": [{"total": -100}]}),
                template,
            ),
            "False",
        )
