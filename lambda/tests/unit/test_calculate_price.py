...
class TestCalculatePrice(unittest.TestCase):
    ...
    def test_equality_count(self):
        price = calculate_price(
            ...
            {
                "acceptTerms": True,
                "contact_name": {"last": "test", "first": "test"},
                "address": {
                    "zipcode": "test",
                    "state": "test",
                    "city": "test",
                ...
                "phone": "7708182022",
                "email": "aramaswamis+12@gmail.com",
                "participants": [
                    {
                        "name": {"last": "test", "first": "test"},
                        "gender": "F",
                        "race": "5K",
                        "age": 16,
                        "shirt_size": "Youth M",
    ...
    def test_more_math(self):
        price = calculate_price(
            "$roundOff * (16 + $total % 5)", {"roundOff": True, "total": 87}
        )
        self.assertEqual(price, 18.0)
        price = calculate_price(
            "$roundOff * (16 + $total % 5)", {"roundOff": False, "total": 87}
        )
        self.assertEqual(price, 0.0)
    ...
    def test_boolean(self):
        price = calculate_price(
            "participants.has_bib_name",
            {
                "participants": [
                    {"has_bib_name": True, "race": "5K - OK"},
                    {"has_bib_name": False, "race": "5K - OK"},
                    {"bib_name": "250", "race": "10K"},
                ]
            },
        )
        self.assertEqual(price, 1.0)
    ...
    def test_array_calc_subtraction(self):
        price = calculate_price(
            "(participants - participants.race:10K) * 25",
            {
                "participants": [
                    {"bib_name": "as", "race": "5K - OK"},
                    {"bib_name": "32", "race": "5K - OK"},
                    {"bib_name": "250", "race": "10K"},
                ]
            },
        )
    ...
    def test_array_calc_subtraction_2(self):
        price = calculate_price(
            "$participants - $participants.age:1",
    ...
    def test_array_item_calculations(self):
        ...
        price = calculate_price(
            "2 * sponsorshipAnnadaanam:300 + sponsorshipAnnadaanam:600", data
        )
    ...
    def test_countarray_multiple(self):
        data = {"children": [{"grade": "7"}, {"grade": "5"}], "paymentMethod": "check"}
    ...
    def test_add_duration(self):
        ...
        self.assertEqual(
            calculate_price("cff_addDuration('2011-07-03', 'P1M')", {}, False),
            "2011-08-03",
        )
        self.assertEqual(
            calculate_price("cff_addDuration('2011-07-03', 'P5D')", {}, False),
            "2011-07-08",
        )
        self.assertEqual(
            calculate_price("cff_addDuration('2011-07-03', 'P1Y')", {}, False),
            "2012-07-03",
        )
    ...
    def test_nthOfNextMonth(self):
        self.assertEqual(
            calculate_price(
                "cff_nthOfNextMonth(CFF_FULL_none, CFF_FULL_none)", {}, False
            ),
            None,
        )
        self.assertEqual(
            calculate_price(
                "cff_nthOfNextMonth('2000-01-01', CFF_FULL_none)", {}, False
            ),
            None,
        )
        self.assertEqual(
            calculate_price("cff_nthOfNextMonth('2000-01-01', 7)", {}, False),
            "2000-02-07",
        )
        self.assertEqual(
            calculate_price(
