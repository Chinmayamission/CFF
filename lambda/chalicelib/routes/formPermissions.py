        ...
        if "cm:cognitoUserPool:" in userIdFull:
            try:
                ...
                user_lookup[userIdFull] = {
                    "name": attributes["name"], "email": attributes["email"], "id": userIdFull}
            # except client.exceptions.UserNotFoundException:
            except Exception:
                user_lookup[userIdFull] = {
                    "name": "unknown", "email": "unknown", "id": userIdFull}
        else:
            user_lookup[userIdFull] = {
                "name": "unknown", "email": "unknown", "id": userIdFull}
    return user_lookup
...
def form_edit_permissions(formId):
    ...
    if "owner" in permissions and permissions["owner"] == True:
    ...
    for i, v in permissions.items():
        ...
        if type(v) is not bool:
            raise Exception(
                "Permission {} not formatted correctly; each value should be a boolean.".format({i: v}))
    ...
    form.save()
    response = {"res": {"permissions": form.cff_permissions},
