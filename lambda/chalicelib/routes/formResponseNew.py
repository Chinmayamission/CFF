...
def form_response_new(formId):
    ...
    if (
        not newResponse
        and not form.formOptions.loginRequired
        and not form.formOptions.responseCanEditByLink
    ):
    ...
    modify_link = getattr(form.formOptions, "modifyLink", app.current_request.json_body.get("modifyLink")) or ""
