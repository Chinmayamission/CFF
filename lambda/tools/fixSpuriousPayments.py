  ...
  for response in responses:
    ...
    for item in response.payment_trail:  
      ...
      if item.method == "paypal_ipn" and item.value["custom"] != str(response.id):
        ...
        idx_payment_status_detail = find_index(response.payment_status_detail, lambda x: x.date == item.date)
        if idx_payment_status_detail == -1:
          idx_payment_status_detail = find_index(response.payment_status_detail, lambda x: (x.date - item.date).total_seconds() < .1)
          print(item.date, response.payment_status_detail, serialize_model(response))
        assert idx_payment_status_detail != -1
        payment_status_detail_item = response.payment_status_detail[idx_payment_status_detail]
        ...
        assert payment_status_detail_item.id == None or payment_status_detail_item.id == item.value["txn_id"]

        assert Response.objects.raw({"payment_trail": {"$elemMatch": {"value.txn_id": item.value["txn_id"] } } } ).count() > 1
...
with open("input-prod.txt", "w+") as file:
  file.writelines([json.dumps(serialize_model(response), indent=4, sort_keys=True) + "\n" for response in responses])
...
for response in responses:
  assert str(sum(float(item.amount) for item in response.payment_status_detail)) >= response.amount_paid or response.paid == False
