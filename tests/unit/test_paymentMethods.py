...
class TestPaymentMethods(unittest.TestCase):
  ...
  def test_simple_join_array_no_refs(self):
    ...
    self.assertEqual(fill_paymentMethods_with_data(paymentMethods, data), expectedOutput)
  def test_refs(self):
  ...
  def test_refs_join_array(self):
    paymentMethods = {"ccavenue": {"name": ["$name.first", " ", "$name.last"], "age": "$age"}}
  ...
  def test_refs_join_array_with_non_string(self):
    paymentMethods = {"ccavenue": {"option": True, "name": ["$name.first", " ", "$name.last"], "age": "$age"}}
    data = {"name": {"first": "John", "last": "Proctor"}, "age": "44"}
    expectedOutput = {"ccavenue": {"option": True, "name": "John Proctor", "age": "44"}}
    self.assertEqual(fill_paymentMethods_with_data(paymentMethods, data), expectedOutput)
