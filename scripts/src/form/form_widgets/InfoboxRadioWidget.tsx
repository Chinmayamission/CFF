...
class RadioWidget extends React.Component<any, { open?: number }> {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        {get(this.props.schema, ["cff:radioDescription"]) && (
          <div style={{ display: "inline-block" }}>
            <Infobox
              i={id + "-1"}
              description={get(this.props.schema, ["cff:radioDescription"])}
            />
          </div>
        )}
        <div
            ...
            const radio = (
              <span>
                ...
                {get(this.props.schema, ["cff:radioDescriptions", i]) && (
                  <Infobox
                    i={id + i}
                    description={get(this.props.schema, [
                      "cff:radioDescriptions",
                      i
                    ])}
                  />
                )}
