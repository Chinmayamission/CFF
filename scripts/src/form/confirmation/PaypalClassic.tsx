...
class PaypalClassic extends React.Component<
  IPaypalClassicProps,
  IPaypalClassicState
> {
  constructor(props: IPaypalClassicProps) {
    ...
    let state: IPaypalClassicState = {
      ...
      return:
        this.props.paymentMethodInfo.redirectUrl ||
      ...
      payButtonText:
        this.props.paymentMethodInfo.payButtonText || "Pay Now with Paypal",
      discount_amount_cart:
        Math.abs(
          sumBy(props.paymentInfo.items, e => (e.amount < 0 ? e.amount : 0))
        ) +
    ...
    if (recurring_item) {
      ...
      let [expr, time, units] = /(\d*)([DWMY])/.exec(
        recurring_item.recurrenceDuration
      );
      state.recurrence = {
        item_name: recurring_item.name,
        a3: String(recurring_item.amount * recurring_item.quantity),
        p3: time,
        t3: units
      };
      if (recurring_item.recurrenceTimes) {
        state.recurrence.srt = recurring_item.recurrenceTimes;
      }
  ...
  render() {
    return (
      <form action={this.state.form_url} method="post">
        ...
        <input
          type="hidden"
          name="cancel_return"
          value={this.state.cancel_return}
        />
        ...
        {this.state.cmd == "_cart" && (
          <input
            ...
            value={this.state.discount_amount_cart}
          />
        )}
        ...
        <input
          type="hidden"
          name="night_phone_a"
          value={this.state.night_phone_a}
        />
        <input
          type="hidden"
          name="night_phone_b"
          value={this.state.night_phone_b}
        />
        <input
          type="hidden"
          name="night_phone_c"
          value={this.state.night_phone_c}
        />
    ...
    alt="PayPal - The safer, easier way to pay online"/>*/}
        <input
          ...
          value={this.state.payButtonText}
        />
        {this.state.cmd == "_xclick-subscriptions" && (
          <div>
            ...
            <input
              type="hidden"
              name="item_name"
              value={this.state.recurrence.item_name}
            />
            ...
            {this.state.recurrence.srt && (
              <input
                type="hidden"
                name="srt"
                value={this.state.recurrence.srt}
              />
            )}
          </div>
        )}
