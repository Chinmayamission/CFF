...
> {
  ...
  editGroups(id, e) {
    let groups = cloneDeep(
      this.props.form.renderedForm.formOptions.dataOptions.groups
    );
  ...
  render() {
    ...
    let dataOptions: IDataOptions = getOrDefaultDataOptions(
      this.props.form.renderedForm
    );
    const dataOptionView: IDataOptionView = find(dataOptions.views, {
      id: this.props.tableViewName
    });
    ...
    if (dataOptionView.groupEdit) {
      const groupOption = find(dataOptions.groups, {
        id: dataOptionView.groupEdit
      });
      return (
        <div>
          <Nav />
          <GroupEdit
            ...
            onSubmit={(i, e) => this.editGroups(i, e)}
          />
        </div>
      );
    }
...
const mapStateToProps = state => ({
  ...state.responses,
  form: state.form,
  tableViewName: (state.router.location.pathname.match(/\/(.[a-zA-Z_]*?)$/) || [
    null,
    null
  ])[1]
});
